...
namespace DrawerLayout_V7_Tutorial
{
    ...
    class MainActivity1: Activity
    {
        ...
        private bool ValidateLogin()
        {
            ...
            var results = service.POST("http://userservice.staging.tangentmicroservices.com:80/api-token-auth/", "{\"username\": \"admin1\",\"password\": \"admin1\"}");
            security = JsonConvert.DeserializeObject<SecurityData>(results);
            var serializer = new JObject();
            //SomeModel model = serializer.Deserialize<SomeModel>(json);

            Console.Write(results.ToString());

            var results2 = service.GET("http://userservice.staging.tangentmicroservices.com:80/api/v1/users/");

            Console.Write(results2.ToString());
